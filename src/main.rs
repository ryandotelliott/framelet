...
impl GraphicsCaptureApiHandler for Capture {
    ...
    fn new(ctx: Context<Self::Flags>) -> Result<Self, Self::Error> {
        println!("Created with Monitor Info: {:?}", ctx.flags);
...
fn main() {
    ...
    // Get monitor dimensions, handling potential errors
    let width = primary_monitor
        .width()
        .expect("Failed to get monitor width");
    let height = primary_monitor
        .height()
        .expect("Failed to get monitor height");
    let name = primary_monitor.name().expect("Failed to get monitor name");

    println!("Primary monitor: {} ({}x{})", name, width, height);
