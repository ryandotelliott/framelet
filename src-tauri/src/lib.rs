...
async fn get_monitors() -> Result<Vec<MonitorInfo>, String> {
    ...
    for (id, monitor) in monitors.iter().enumerate() {
        let name = monitor.name().map_err(|e| format!("Failed to get monitor name: {}", e))?;
...
async fn stop_recording() -> Result<String, String> {
    ...
    if let Some(stop_signal) = &state.stop_signal {
        ...
        if let Some(thread) = state.recording_thread.take() {
            ...
            thread.join().map_err(|_| "Failed to join recording thread")?;
...
pub fn run() {
    tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![greet, get_monitors, start_recording, stop_recording])
